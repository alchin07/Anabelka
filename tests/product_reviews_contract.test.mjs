...
const productReviewsPartial = read('views/product/partials/reviews.php');
...
assert.match(reviewModel, /hasReview|1062/i);
...
assert.match(productController, /catch\s*\(Throwable\s+\$e\)[\s\S]*?Product reviews/i);

assert.match(productView, /css\/reviews\.css/);
assert.match(productView, /partials\/reviews\.php/);
assert.match(productReviewsPartial, /id="product-reviews"/);
assert.match(productReviewsPartial, /action="\/Anabelka\/product\/[\s\S]*?\/reviews"/);
assert.match(productReviewsPartial, /name="_csrf"/);
assert.match(productReviewsPartial, /name="rating"/);
assert.match(productReviewsPartial, /name="body"/);
assert.match(productReviewsPartial, /htmlspecialchars|\$reviewEscape/);
assert.doesNotMatch(productReviewsPartial, /<\?=\s*\$review\[['"]body['"]\]\s*\?>/);
