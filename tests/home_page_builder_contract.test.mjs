...
assert.match(home, /anabelka-builder-preview\.css\?v=2/);
assert.match(home, /anabelka-builder-preview\.js\?v=3/);
...
assert.doesNotMatch(previewJs, /elementFromPoint/);
assert.match(previewJs, /const\s+midpoint\s*=\s*rect\.top\s*\+\s*\(rect\.height\s*\/\s*2\)/);
assert.match(previewJs, /parent\.appendChild\(state\.wrapper\)/);
