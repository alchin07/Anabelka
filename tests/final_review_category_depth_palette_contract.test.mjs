...
const adminColors = [
    backgroundHex(ruleBody(adminCss, '.category-admin-card'), 'admin parent'),
    backgroundHex(ruleBody(adminCss, '.category-admin-row[data-category-level="1"] .category-admin-card'), 'admin Test 1'),
    backgroundHex(ruleBody(adminCss, '.category-admin-row[data-category-level="2"] .category-admin-card'), 'admin Test 2'),
    backgroundHex(ruleBody(adminCss, '.category-admin-row[data-category-level="3"] .category-admin-card'), 'admin Test 3'),
    backgroundHex(ruleBody(adminCss, '.category-admin-row[data-category-level="4"] .category-admin-card'), 'admin Test 4')
];
assertStrictlyDarker(adminColors, 'admin category depth');
assert.match(
    adminCss,
    /\.category-admin-row\[data-category-level="5"\][\s\S]*?background:\s*#[0-9a-f]{6}/i,
    'levels deeper than Test 4 must remain explicitly bounded to the deepest palette'
);

const publicColors = [
    backgroundHex(ruleBody(sidebarCss, '.public-catalog-sidebar-link'), 'public parent'),
    backgroundHex(ruleBody(sidebarCss, '.public-catalog-sidebar-row[data-level="2"]\n        .public-catalog-sidebar-link'), 'public Test 1'),
    backgroundHex(ruleBody(sidebarCss, '.public-catalog-sidebar-row[data-level="3"]\n        .public-catalog-sidebar-link'), 'public Test 2'),
    backgroundHex(ruleBody(sidebarCss, '.public-catalog-sidebar-row[data-level="4"]\n        .public-catalog-sidebar-link'), 'public Test 3'),
    backgroundHex(ruleBody(sidebarCss, '.public-catalog-sidebar-row[data-level="5"]\n        .public-catalog-sidebar-link'), 'public Test 4')
];
